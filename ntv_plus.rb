...
item_ids.close
item_ids = File.open("item_ids.txt", "r")

CSV.open("ntv_demo.csv", "a") do |csv_data|
  ...
  item_ids.each_line do |line|
    ...
    @response['items'].each do |id|
      csv_data << ["#{DateTime.strptime(date, '%s').to_s.split('T').first}", "https://vk.com/ntv_plus?w=wall#{owner_id}_#{item_id}", "https://vk.com/id#{id}"]
    end
